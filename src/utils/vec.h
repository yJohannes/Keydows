...
struct Vec2
{
    ...
    static constexpr T distance(const Vec2 &a, const Vec2 &b) { return (b - a).length(); }
    static constexpr Vec2 lerp(const Vec2 &a, const Vec2 &b, T t) { return a + (b - a) * t; }
    constexpr Vec2 reflect(const Vec2 &normal) const { return *this - normal * (2 * dot(normal)); }
    constexpr Vec2 project(const Vec2 &v) const { return v * (dot(v) / v.length_squared()); }
